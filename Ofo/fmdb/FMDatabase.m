...
//        YJLog(@"error opening!: %d", err);
    ...
    return YES;
#else
//    YJLog(@"openWithFlags requires SQLite 3.5");
    return NO;
#endif
}
...
//                    YJLog(@"Closing leaked statement");
...
//            YJLog(@"WARNING: Requested sleep of %i milliseconds, but SQLite returned %i. Maybe SQLite wasn't built with HAVE_USLEEP=1?", requestedSleepInMillseconds, actualSleepInMilliseconds);
        }
        return 1;
    }
...
//    YJLog(@"%s:%d", __FUNCTION__, __LINE__);
//    YJLog(@"FMDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
    return -1;
}
...
#pragma unused(i)
//    YJLog(@"%s:%d", __FUNCTION__, __LINE__);
//    YJLog(@"FMDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (BOOL)rekeyWithData:(NSData *)keyData {
    ...
    if (rc != SQLITE_OK) {
//        YJLog(@"error on rekey: %d", rc);
//        YJLog(@"%@", [self lastErrorMessage]);
    }
...
- (void)warnInUse {
//    YJLog(@"The FMDatabase %@ is currently in use.", self);
...
//        YJLog(@"The FMDatabase %@ is not open.", self);
...
//        YJLog(@"%@ executeQuery: %@", self, sql);
    ...
    if (!pStmt) {
        ...
        if (SQLITE_OK != rc) {
            if (_logsErrors) {
//                YJLog(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
//                YJLog(@"DB Query: %@", sql);
//                YJLog(@"DB Path: %@", _databasePath);
            }
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            if (_traceExecution) {
//                YJLog(@"%@ = %@", parameterName, [dictionaryArgs objectForKey:dictionaryKey]);
            }
...
//                YJLog(@"Could not find index for %@", dictionaryKey);
            }
        }
    }
    else {
...
//                    YJLog(@"data: %ld bytes", (unsigned long)[(NSData*)obj length]);
                }
                else {
//                    YJLog(@"obj: %@", obj);
                }
    ...
    if (idx != queryCount) {
//        YJLog(@"Error: the bind count is not correct for the # of variables (executeQuery)");
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return nil;
    }
...
//            YJLog(@"%@", message);
    ...
    if (SQLITE_DONE == rc) {
        ...
        if (_logsErrors) {
//            YJLog(@"Error calling sqlite3_step. Query was interrupted (%d: %s) SQLITE_INTERRUPT", rc, sqlite3_errmsg(_db));
//            YJLog(@"DB Query: %@", sql);
        }
    }
...
//            YJLog(@"DB Query: %@", sql);
        ...
        if (SQLITE_ERROR == rc) {
            if (_logsErrors) {
//                YJLog(@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc, sqlite3_errmsg(_db));
//                YJLog(@"DB Query: %@", sql);
            }
        }
            ...
            // uh oh.
            if (_logsErrors) {
//                YJLog(@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc, sqlite3_errmsg(_db));
//                YJLog(@"DB Query: %@", sql);
            }
        }
        else {
            // wtf?
            if (_logsErrors) {
//                YJLog(@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(_db));
//                YJLog(@"DB Query: %@", sql);
            }
        }
    }
    ...
    if (closeErrorCode != SQLITE_OK) {
        if (_logsErrors) {
//            YJLog(@"Unknown error finalizing or resetting statement (%d: %s)", closeErrorCode, sqlite3_errmsg(_db));
//            YJLog(@"DB Query: %@", sql);
        }
    }
...
//        YJLog(@"Error inserting batch: %s", errmsg);
        sqlite3_free(errmsg);
    }
