...
//        YJLog(@"Warning: There seem to be no columns in this set.");
    }
...
//        YJLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [_parentDB databasePath]);
//        YJLog(@"Database busy");
        if (outErr) {
...
//        YJLog(@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
        if (outErr) {
            *outErr = [_parentDB lastError];
        }
    }
...
//        YJLog(@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
        ...
        // wtf?
//        YJLog(@"Unknown error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
        if (outErr) {
            *outErr = [_parentDB lastError];
        }
    }
...
//    YJLog(@"Warning: I could not find the column named '%@'.", columnName);
